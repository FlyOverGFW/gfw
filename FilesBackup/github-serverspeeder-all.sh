#!/bin/bash
# From https://gist.github.com/rptec/69e2d10ef2e30842eea4
...
if [ $? == 1 ]; then
	...
	if [ "$release" == "CentOS" ]; then
		...
		cat serverspeederbin.txt | grep  "$release/$ver1/$ver21-$ver22[^/]*/$ver3/"  | awk -F '/' '{ print "请选择"NR"："$3 }'
	fi
	...
	cver2str="cat serverspeederbin.txt | grep  \"$release/$ver1/$ver21-$ver22[^/]*/$ver3/\"  | awk -F '/' '{ print \"请选择\"NR\"：\"\$3 }' | awk -F '：' '/"$cver2："/{ print \$2 }'"
	ver2=$(eval $cver2str)
	 if [ "$ver2" == "" ]; then
                exit 1
        fi
...
if [ $? == 1 ]; then
	...
	cat serverspeederbin.txt | grep  "$release/$ver1/$ver2/$ver3/"  | awk -F '/' '{ print "请选择"NR"："$5 }'
	echo "请选择锐速版本号（输入数字序号）："
        read cver4
	if [ "$cver4" == "" ]; then
		exit 1
	fi
     	cver4str="cat serverspeederbin.txt | grep  \"$release/$ver1/$ver2/$ver3/\"  | awk -F '/' '{ print \"请选择\"NR\"：\"\$5 }' | awk -F '：' '/"$cver4："/{ print \$2 }'"
        ver4=$(eval $cver4str)
	 if [ "$ver4" == "" ]; then
                exit 1
        fi
